...
Runs the full Rapidly stack (API, worker, web, DB, Redis, MinIO) in Docker
...
def _read_stored_instance() -> int | None:
    """Read RAPIDLY_DOCKER_INSTANCE from .env.docker if set."""
    ...
    for line in ENV_FILE.read_text().splitlines():
        ...
        if line.startswith("#") or not line:
            continue
        match = re.match(r"^RAPIDLY_DOCKER_INSTANCE\s*=\s*(\d+)\s*$", line)
        if match:
...
def _write_stored_instance(instance: int) -> None:
    """Write RAPIDLY_DOCKER_INSTANCE to .env.docker."""
    _ensure_env_file()
    content = ENV_FILE.read_text()
    # Replace existing line (commented or not)
    new_line = f"RAPIDLY_DOCKER_INSTANCE={instance}"
    if re.search(r"^#?\s*RAPIDLY_DOCKER_INSTANCE\s*=", content, re.MULTILINE):
        content = re.sub(
            r"^#?\s*RAPIDLY_DOCKER_INSTANCE\s*=.*$",
            new_line,
            content,
            flags=re.MULTILINE,
        )
...
def _clear_stored_instance() -> bool:
    """Comment out RAPIDLY_DOCKER_INSTANCE in .env.docker. Returns True if found."""
    if not ENV_FILE.exists():
        return False
    content = ENV_FILE.read_text()
    if re.search(r"^RAPIDLY_DOCKER_INSTANCE\s*=", content, re.MULTILINE):
        content = re.sub(
            r"^RAPIDLY_DOCKER_INSTANCE\s*=.*$",
            "# RAPIDLY_DOCKER_INSTANCE=",
            content,
            flags=re.MULTILINE,
        )
        ENV_FILE.write_text(content)
...
def _detect_instance() -> tuple[int, str]:
    ...
    Priority:
    1. RAPIDLY_DOCKER_INSTANCE in .env.docker
...
def _ensure_server_env() -> None:
    ...
    Applies central secrets from ~/.config/rapidly/secrets.env when available,
    ...
    if "RAPIDLY_STRIPE_PUBLISHABLE_KEY" in central_secrets:
        central_secrets["NEXT_PUBLIC_STRIPE_KEY"] = central_secrets[
            "RAPIDLY_STRIPE_PUBLISHABLE_KEY"
        ]
    if "RAPIDLY_GITHUB_APP_NAMESPACE" in central_secrets:
        central_secrets["NEXT_PUBLIC_GITHUB_APP_NAMESPACE"] = central_secrets[
            "RAPIDLY_GITHUB_APP_NAMESPACE"
        ]

    # Write .env with restricted permissions (secrets inside).
    fd = os.open(SERVER_ENV_FILE, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
...
def _build_compose_cmd(instance: int, monitoring: bool = False) -> list[str]:
    ...
    project_name = f"rapidly-dev-{instance}"
    cmd = [
        "docker", "compose",
        "-p", project_name,
        "-f", str(COMPOSE_FILE),
...
def _print_access_info(ctx: typer.Context, instance: int, monitoring: bool = False) -> None:
    ...
    offset = instance * 100
    console.print()
    console.print("[bold]Rapidly Docker Development Environment[/bold]")
    ...
    if monitoring:
        ...
        console.print(f"  Grafana:       http://localhost:{3001 + offset} (admin/rapidly)")
    console.print()
...
def register(app: typer.Typer, prompt_setup: callable) -> None:
    ...
    def docker_up(
        ...
        console.print(f"\n[bold blue]Starting Rapidly Docker environment (instance {instance})[/bold blue]\n")
    ...
    def docker_down(
        ...
        console.print(f"[dim]Stopping Rapidly Docker environment (instance {instance})...[/dim]")
    ...
    def docker_cleanup(
        ...
        console.print(f"[dim]Cleaning up Rapidly Docker environment (instance {instance})...[/dim]")
        result = run_command(cmd, env=env)
        if result and result.returncode == 0:
