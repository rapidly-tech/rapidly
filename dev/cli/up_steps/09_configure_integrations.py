...
def is_github_configured() -> bool:
    ...
    if SECRETS_FILE.exists():
        ...
        if "RAPIDLY_GITHUB_CLIENT_ID=" in content:
            for line in content.split("\n"):
                if line.startswith("RAPIDLY_GITHUB_CLIENT_ID="):
...
def is_github_skipped() -> bool:
    ...
    if SECRETS_FILE.exists():
        for line in SECRETS_FILE.read_text().split("\n"):
            if line.startswith("RAPIDLY_SKIP_GITHUB_SETUP="):
                value = line.split("=", 1)[1].strip().strip("\"'")
...
def set_github_skipped(skipped: bool = True) -> None:
    ...
    _update_secrets_file("RAPIDLY_SKIP_GITHUB_SETUP", "true" if skipped else None)
...
def is_stripe_configured() -> bool:
    ...
    if SECRETS_FILE.exists():
        ...
        for line in content.split("\n"):
            if line.startswith("RAPIDLY_STRIPE_SECRET_KEY="):
                value = line.split("=", 1)[1].strip().strip("\"'")
                has_secret = bool(value)
            if line.startswith("RAPIDLY_STRIPE_PUBLISHABLE_KEY="):
                value = line.split("=", 1)[1].strip().strip("\"'")
...
def is_stripe_skipped() -> bool:
    ...
    if SECRETS_FILE.exists():
        for line in SECRETS_FILE.read_text().split("\n"):
            if line.startswith("RAPIDLY_SKIP_STRIPE_SETUP="):
                value = line.split("=", 1)[1].strip().strip("\"'")
                return value.lower() == "true"
    return False
...
def set_stripe_skipped(skipped: bool = True) -> None:
    ...
    _update_secrets_file("RAPIDLY_SKIP_STRIPE_SETUP", "true" if skipped else None)
...
def save_stripe_keys(secret_key: str, publishable_key: str, webhook_secret: str = "") -> None:
    ...
    _update_secrets_file("RAPIDLY_STRIPE_SECRET_KEY", secret_key)
    _update_secrets_file("RAPIDLY_STRIPE_PUBLISHABLE_KEY", publishable_key)
    if webhook_secret:
        _update_secrets_file("RAPIDLY_STRIPE_WEBHOOK_SECRET", webhook_secret)
...
def _update_secrets_file(key: str, value: str | None) -> None:
    ...
    with open(SECRETS_FILE, "w") as f:
        f.write("# Rapidly Development Secrets\n")
