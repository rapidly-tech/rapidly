#!/bin/bash
# Startup script for Rapidly API and Worker services in development mode
...
options = {'kid': 'rapidly_dev', 'use': 'sig'}
...
max_attempts=30
attempt=0
while ! pg_isready -h "$RAPIDLY_POSTGRES_HOST" -p "$RAPIDLY_POSTGRES_PORT" -U "$RAPIDLY_POSTGRES_USER" -q; do
    attempt=$((attempt + 1))
...
import asyncio
from rapidly.postgres import create_async_engine
...
case "${1:-api}" in
    api)
        ...
        exec uv run uvicorn rapidly.app:app \
            --reload \
            --reload-dir rapidly \
            --host 0.0.0.0 \
            --port 8000 \
    ...
    worker)
        ...
        exec uv run dramatiq \
            ...
            --watch rapidly \
            -f rapidly.worker.scheduler:start \
            rapidly.worker.run
        ;;
    shell)
        echo "Starting shell..."
