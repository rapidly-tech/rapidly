...
export default function FileSharingLanding({
  ...
  // Track first render vs state transitions.
  // First render: CSS fade-in. Subsequent: framer-motion.
  const isFirstRender = useRef(true)
  const shouldAnimate = !isFirstRender.current
  if (isFirstRender.current && state.type !== 'initial') {
  ...
  const initialMotion = shouldAnimate ? stateTransition.initial : false
    ...
    <AnimatePresence mode="wait" initial={false}>
      {state.type === 'initial' && isProcessing && (
        <motion.div
          ...
          animate={stateTransition.animate}
          exit={stateTransition.exit}
        >
      ...
      {state.type === 'initial' && !isProcessing && (
        <motion.div
          key="initial"
          className={
            !shouldAnimate && entranceAnimation ? 'animate-fade-in-up' : ''
          }
          initial={initialMotion}
          animate={stateTransition.animate}
          exit={stateTransition.exit}
