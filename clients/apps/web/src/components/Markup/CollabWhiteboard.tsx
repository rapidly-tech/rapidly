...
import { align, distribute } from '@/utils/markup/align'
import { makeAlignmentGuidesOverlay } from '@/utils/markup/alignment-guides-overlay'
import { clearCanvas } from '@/utils/markup/clear-canvas'
import {
  ...
  getClipboard,
  serialiseSelection,
} from '@/utils/markup/clipboard'
import { type Command } from '@/utils/markup/command-palette'
import { copyElementsAsPng } from '@/utils/markup/copy-as-image'
import { makeCursorOverlay } from '@/utils/markup/cursor-overlay'
import { cycleNext, cyclePrev } from '@/utils/markup/cycle-selection'
import { makeDimensionsOverlay } from '@/utils/markup/dimensions-overlay'
import {
  displayName as displayElementName,
  setName as setElementName,
} from '@/utils/markup/element-name'
import {
  createElementStore,
  type ElementStore,
} from '@/utils/markup/element-store'
import { EMBED_SANDBOX, isEmbeddableUrl } from '@/utils/markup/embed-allowlist'
import {
  computeBounds,
  downloadBlob,
  exportToJSON,
  exportToPNG,
} from '@/utils/markup/export'
import { findNext, findPrevious } from '@/utils/markup/find-next'
import { flipHorizontal, flipVertical } from '@/utils/markup/flip'
import {
  createFollowMeController,
  type FollowMeController,
} from '@/utils/markup/follow-me'
import {
  exportFrameAsJson,
  exportFrameAsPng,
  exportFrameAsSvg,
} from '@/utils/markup/frame-export'
import { expandToGroups, group, ungroup } from '@/utils/markup/groups'
import { hasLink, setLink } from '@/utils/markup/hyperlinks'
import {
  createImageElement,
  extractImageFile,
  extractPastedImage,
} from '@/utils/markup/image-paste'
import {
  importScene,
  isImportError,
  parseExportedScene,
} from '@/utils/markup/import-json'
import {
  createInstallPromptController,
  type InstallPromptController,
} from '@/utils/markup/install-prompt'
import {
  ageToAlpha,
  createLaserState,
  type LaserController,
} from '@/utils/markup/laser'
import { makeLaserOverlay } from '@/utils/markup/laser-overlay'
import { saveTemplate } from '@/utils/markup/library'
import { filterUnlocked, toggleLock } from '@/utils/markup/locks'
import {
  detectMermaidChartType,
  mermaidToElements,
  parseMermaid,
} from '@/utils/markup/mermaid'
import {
  blockDiagramToElements,
  parseBlockDiagram,
} from '@/utils/markup/mermaid-block'
import { c4ToElements, parseC4 } from '@/utils/markup/mermaid-c4'
import {
  classDiagramToElements,
  parseClassDiagram,
} from '@/utils/markup/mermaid-class'
import { erDiagramToElements, parseErDiagram } from '@/utils/markup/mermaid-er'
import { ganttToElements, parseGantt } from '@/utils/markup/mermaid-gantt'
import {
  gitGraphToElements,
  parseGitGraph,
} from '@/utils/markup/mermaid-gitgraph'
import { journeyToElements, parseJourney } from '@/utils/markup/mermaid-journey'
import { mindmapToElements, parseMindmap } from '@/utils/markup/mermaid-mindmap'
import { packetToElements, parsePacket } from '@/utils/markup/mermaid-packet'
import { parsePie, pieToElements } from '@/utils/markup/mermaid-pie'
import {
  parseQuadrantChart,
  quadrantChartToElements,
} from '@/utils/markup/mermaid-quadrant'
import {
  parseRequirementDiagram,
  requirementDiagramToElements,
} from '@/utils/markup/mermaid-requirement'
import { parseSankey, sankeyToElements } from '@/utils/markup/mermaid-sankey'
import {
  parseSequence,
  sequenceToElements,
} from '@/utils/markup/mermaid-sequence'
import {
  parseStateDiagram,
  stateDiagramToElements,
} from '@/utils/markup/mermaid-state'
import {
  parseTimeline,
  timelineToElements,
} from '@/utils/markup/mermaid-timeline'
import { parseXYChart, xyChartToElements } from '@/utils/markup/mermaid-xychart'
import { parseZenUml, zenUmlToElements } from '@/utils/markup/mermaid-zenuml'
import { deltaFromArrowKey, nudge } from '@/utils/markup/nudge'
import { digitToOpacity } from '@/utils/markup/opacity-quickkey'
import {
  createPinchPanGesture,
  type PinchPanGesture,
} from '@/utils/markup/pinch-gesture'
import {
  createPointerPreference,
  HANDLE_SIZE_FOR_PRECISION,
  type PointerPrecision,
} from '@/utils/markup/pointer-preference'
import {
  inMemoryPresenceSource,
  type InMemoryPresenceSource,
  type PresenceSource,
  type PresenceUser,
} from '@/utils/markup/presence'
import {
  advanceFrame,
  computeFrames,
  viewportForBounds,
} from '@/utils/markup/presentation'
import { applyToSelection } from '@/utils/markup/properties'
import { makeRemoteSelectionOverlay } from '@/utils/markup/remote-selection-overlay'
import { Renderer } from '@/utils/markup/renderer'
import { applyReplacements } from '@/utils/markup/replace-text'
import {
  rotate90Clockwise,
  rotate90CounterClockwise,
} from '@/utils/markup/rotate-by'
import { makeScrollbarsOverlay } from '@/utils/markup/scrollbars'
import {
  selectSameFillColor,
  selectSameFontFamily,
  selectSameStrokeColor,
  selectSameType,
} from '@/utils/markup/select-same'
import { SelectionState } from '@/utils/markup/selection'
import { makeSelectionOverlay } from '@/utils/markup/selection-overlay'
import {
  copyStyle as copyElementStyle,
  pasteStyle as pasteElementStyle,
} from '@/utils/markup/style-clipboard'
import { exportToSVG } from '@/utils/markup/svg-export'
import { swapStrokeAndFill } from '@/utils/markup/swap-colors'
import {
  parseClipboardText,
  readSystemClipboardPayload,
  writeSystemClipboard,
} from '@/utils/markup/system-clipboard'
import { onEditRequest } from '@/utils/markup/text-editing'
import { toolIdForKey } from '@/utils/markup/tool-keys'
import {
  currentLassoPath,
  currentMarqueeRect,
  currentSnapGuides,
  ...
  type ToolCtx,
  type ToolId,
} from '@/utils/markup/tools'
import { createUndoManager, type UndoController } from '@/utils/markup/undo'
import {
  isReadOnlyPaletteCommand,
  isReadOnlyTool,
  isViewModeShortcutAllowed,
  withViewModeUrl,
} from '@/utils/markup/view-mode'
import { makeViewport, zoomAt, type Viewport } from '@/utils/markup/viewport'
import { animateViewport } from '@/utils/markup/viewport-transitions'
import { setHidden, toggleHidden } from '@/utils/markup/visibility'
import {
  bringForward,
  bringToFront,
  sendBackward,
  sendToBack,
} from '@/utils/markup/z-order'
import {
  viewportForKeyboardZoom,
  zoomDirectionForKey,
} from '@/utils/markup/zoom-keyboard'
import { zoomToFit, zoomToSelection } from '@/utils/markup/zoom-to-fit'
    ...
    <div className="flex h-full w-full flex-col bg-slate-50 dark:bg-slate-950">
      ...
      <LibraryDialog
        ...
        onInsert={(template) => {
          ...
          import('@/utils/markup/library').then(({ insertTemplate }) => {
