...
import { pathFor as embedPath, paintEmbed } from './embed'
...
const REGISTRY: Partial<Record<ElementType, ShapeAdapter>> = {
  ...
  embed: {
    pathFor: (el) => embedPath(el as Parameters<typeof embedPath>[0]),
    paint: (ctx, el, path) =>
  ...
  'image-underlay': {
    pathFor: (el) =>
      imageUnderlayPath(el as Parameters<typeof imageUnderlayPath>[0]),
    paint: (ctx, el, path) =>
      paintImageUnderlay(
        ctx,
        el as Parameters<typeof paintImageUnderlay>[1],
        path,
      ),
  },
}
