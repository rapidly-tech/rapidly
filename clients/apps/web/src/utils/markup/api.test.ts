...
describe('createSession', () => {
  it('POSTs to /api/v1/markup/session with max_participants + kind', async () => {
      ...
      () =>
        new Response(
          JSON.stringify({
            ...
            long_slug: 'abcd-efgh',
            secret: 'sec',
            invite_template: '/markup/abc?t={token}',
    ...
    expect(calls[0]!.url).toBe('/api/v1/markup/session')
...
describe('mintInvite', () => {
  it('POSTs the host secret to /session/{slug}/invite', async () => {
    ...
    expect(out.invite_url).toBe('/markup/abc?t=tok')
    expect(calls[0]!.url).toBe('/api/v1/markup/session/abc/invite')
    expect(calls[0]!.init?.method).toBe('POST')
    expect(JSON.parse(calls[0]!.init!.body as string)).toEqual({
      secret: 'the-secret',
    })
...
describe('getPublicView', () => {
  it('GETs session metadata and returns it typed', async () => {
    ...
    expect(calls[0]!.url).toBe('/api/v1/markup/session/abc')
