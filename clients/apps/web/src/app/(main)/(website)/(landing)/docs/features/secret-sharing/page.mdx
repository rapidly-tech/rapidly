export const metadata = {
  title: 'Secret Sharing | Rapidly Docs',
  description:
    'Share passwords, API keys, and sensitive files with one-time encrypted links that self-destruct after a single view',
}

# Secret Sharing

<p className="docs-lead">
  Share passwords, API keys, and sensitive files with one-time encrypted links
  that self-destruct after a single view
</p>
...
<Note>
  The decryption key lives exclusively in the URL fragment. It is never included
  in HTTP requests, server logs, or analytics. The server only ever stores and
  returns OpenPGP ciphertext.
</Note>
