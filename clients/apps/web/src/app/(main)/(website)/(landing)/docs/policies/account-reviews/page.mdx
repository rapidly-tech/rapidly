export const metadata = {
  title: 'Account Reviews | Rapidly Docs',
  description: 'How account reviews work on Rapidly',
}

# Account Reviews

<p className="docs-lead">How account reviews work on Rapidly</p>

We need to make sure that businesses using Rapidly comply with our [acceptable products & use](/docs/policies/acceptable-use) policies. Combined with continuously monitoring, reviewing and preventing fraud, unacceptable use, bad actors and high risk accounts.
...
1. A quick survey about your business, products and intended use case with Rapidly.
...
We need to perform this review to ensure compliance with our [acceptable products & use](/docs/policies/acceptable-use) policy. Combined with meeting our own KYC/AML requirements as a file-sharing platform.
...
You'll get notified over email that a review is taking place. Payouts will be paused during this time, but it has no impact on your customers' experience or ability to purchase or checkout at any time.
...
To maintain platform integrity and ensure smooth operations, we have established clear guidelines for merchants using Rapidly.
...
- **Use Free Shares for Testing**: Use a free share to test file sharing and webhook flows without real money.
...
- **Consequences of High Chargeback Rates**:
  ...
  <Note>
    We also have integrations with credit card networks to receive early
    chargeback signals before they're officially filed. We automatically refund
    such transactions under a certain value and revoke any file share access
    associated with the customer to reduce chargebacks proactively.
  </Note>

#### Policy Violations

For merchants who violate our [acceptable use policies](/docs/policies/acceptable-use) (and don't have high chargeback rates):
...
- **Resolution Process**:
  ...
  - We have to revoke file share access and refund payments made in violation of our acceptable use policies for compliance and risk, and strive to do so in collaboration with the merchant.
...
- Ensure compliance with our [acceptable use policies](/docs/policies/acceptable-use)
...
To help us verify that everything is working correctly in line with our acceptable use policy, our team would ask you to share a free access link by email. This is our preferred method, as it allows the team to go through the full journey themselves and confirm the automated fulfillment from an unpaid user to a paid user.
...
### How does changing the admin of a workspace work?

To change the admin of a workspace:

1. Invite the new admin to the team via `Settings` > `Members` in the Rapidly dashboard
2. Ask that new admin to complete identity verification under `Finance` > `Account` after logging in via that email in the Rapidly dashboard
...
If you need assistance with changing workspace ownership or have special circumstances, please contact [support@rapidly.tech](mailto:support@rapidly.tech).
