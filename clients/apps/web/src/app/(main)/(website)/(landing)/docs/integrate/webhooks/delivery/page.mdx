export const metadata = {
  title: 'Handle & monitor webhook deliveries | Rapidly Docs',
...
<p className="docs-lead">
  How to parse, validate and handle webhooks and monitor their
</p>
...
You now need to setup a route handler for the endpoint registered on Rapidly to
receive, validate and parse webhooks before handling them according to your
needs.

### Using the Standard Webhooks library

You can use the [Standard Webhooks](https://www.standardwebhooks.com/) library to validate and parse webhook events. See full examples below.
...
try {
const secret = process.env['RAPIDLY_WEBHOOK_SECRET'] ?? ''
...
const wh = new Webhook(base64Secret)
      ...
      const event = wh.verify(req.body.toString(), headers)
...
def webhook():
    try:
        secret = os.getenv('RAPIDLY_WEBHOOK_SECRET', '')
        base64_secret = base64.b64encode(secret.encode('utf-8')).decode('utf-8')
        wh = Webhook(base64_secret)

        event = wh.verify(request.data.decode('utf-8'), dict(request.headers))
...
Both examples above expect an environment variable named `RAPIDLY_WEBHOOK_SECRET`
to be set to the secret you configured during the endpoint setup.

<Info>
  **Note: Secret needs to be base64 encoded**

The Standard Webhooks library expects the secret to be base64 encoded. The examples above handle this encoding for you.
...
- Admin of the workspace will receive an email notification.

To re-enable a disabled endpoint, go to your workspace's webhook settings in the dashboard and manually enable it. Before re-enabling, ensure your endpoint is properly configured and reachable to avoid repeated disabling.

## Troubleshooting

### Not receiving webhooks

Seeing deliveries on Rapidly, but not receiving them on your end? Below are some
...
- Try adding trailing `/` to the URL on Rapidly. Often `/foo` is resolved to
  `/foo/` by frameworks.

`HTTP 3xx`

Redirect responses (301, 302, 307, etc.) are treated as failures. Rapidly does not follow redirects for webhook deliveries. Update your webhook URL to the final destination URL to avoid redirects.
