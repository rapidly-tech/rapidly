export const metadata = {
  title: 'OAuth 2.0 Connect | Rapidly Docs',
  description: '',
}
...
```
https://rapidly.tech/oauth2/authorize?
  response_type=code
  &client_id=CLIENT_ID
...
<ParamField path="response_type=code" type="string" required>
  ...
  the only one supported by Rapidly.
</ParamField>
...
If you redirect the user to this URL, they'll see a page asking them to grant access to their data, corresponding to the scopes you asked for. By default, the user will also be prompted to select one of their workspaces, as Rapidly generates workspace-level access tokens by default.

If they allow it and select a workspace, they'll be redirected to your `redirect_uri` with a `code` parameter in the query string. This code is a one-time code that you can exchange for an access token.

To skip the workspace selection and get a user-scoped token instead, you can add `sub_type=user` to the authorization URL:
...
```bash Terminal
curl -X POST https://api.rapidly.tech/api/oauth2/token \
...
{
  "token_type": "Bearer",
  "access_token": "rapidly_at_XXX",
  "expires_in": 864000,
  "refresh_token": "rapidly_rt_XXX",
  "scope": "openid email",
  "id_token": "ID_TOKEN"
}
```
...
#### Workspace vs User Access Tokens

By default, Rapidly OAuth2 flow generates **workspace-level** access tokens. These tokens are tied to a specific workspace rather than a user, allowing requests to operate only on that workspace's data, which improves privacy and security.

When using the default flow, the user will be prompted to select one of their workspaces before allowing access to their data:

```
https://rapidly.tech/oauth2/authorize?response_type=code&client_id=rapidly_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email
```
...
```
https://rapidly.tech/oauth2/authorize?response_type=code&client_id=rapidly_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email&sub_type=user
```

The rest of the flow remains unchanged. The access token you'll get will be tied to either the selected workspace (default) or the user (when explicitly requested).
...
```bash Terminal
curl -X GET https://api.rapidly.tech/api/oauth2/userinfo \
  -H 'Authorization: Bearer rapidly_at_XXX'
```
