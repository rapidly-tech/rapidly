...
export const useWorkspaceAccount = (
  workspaceId?: string,
): UseQueryResult<WorkspaceAccount> => {
  const { session } = useSession()
  return useQuery({
    queryKey: ['finance', 'account', workspaceId],
    queryFn: () =>
      fetch(
        `${process.env.EXPO_PUBLIC_RAPIDLY_SERVER_URL}/api/workspaces/${workspaceId}/account`,
        {
          headers: {
...
export enum PlatformFeeType {
   ...
   * to a different country than Rapidly's.
   */
...
export const useTransactionsSummary = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetch(
        `${process.env.EXPO_PUBLIC_RAPIDLY_SERVER_URL}/api/transactions/summary?account_id=${accountId}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${session}`,
...
export const usePayoutEstimate = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetch(
        `${process.env.EXPO_PUBLIC_RAPIDLY_SERVER_URL}/api/payouts/estimate?account_id=${accountId}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${session}`,
...
export const useCreatePayout = (
  ...
  return useMutation({
    mutationFn: () =>
      fetch(`${process.env.EXPO_PUBLIC_RAPIDLY_SERVER_URL}/api/payouts/`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${session}`,
        },
  ...
  return useQuery({
    ...
    queryFn: () =>
      queryClient
        .getQueryData<{
          items: Payout[]
          pagination: schemas['PageMeta']
        }>(['finance', 'payouts'])
        ?.items.find((payout: Payout) => payout.id === payoutId),
    enabled: !!payoutId,
  })
}
