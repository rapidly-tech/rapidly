#!/usr/bin/env node
/**
 * Codemod: replace <p> and <span> with <Text /> from @rapidly-tech/orbit
 *
 * Usage:
 ...
 *   </span>        →  </Text>
 *
 * The import { Text } from '@rapidly-tech/orbit' is added/updated automatically.
 *
...
/**
 * Ensures `Text` is imported from `@rapidly-tech/orbit` in the given source string.
 * - If an import from `@rapidly-tech/orbit` already exists and includes `Text` → no-op
 * - If an import from `@rapidly-tech/orbit` exists without `Text` → adds `Text` to it
...
function ensureTextImport(source, filePath) {
  ...
  for (const stmt of sourceFile.statements) {
    if (
      ...
      stmt.moduleSpecifier.text === '@rapidly-tech/orbit'
    ) {
  ...
  // No @rapidly-tech/orbit import — add one.
  ...
  const importLine = `import { Text } from '@rapidly-tech/orbit'\n`
