...
The Rapidly development environment runs as a Docker Compose stack with:
...
Includes Prometheus (port 9090) and Grafana (port 3001, login: admin/rapidly).
...
- Uses project name `rapidly-dev-{instance}`
...
```bash
psql -U rapidly -d rapidly        # Connect to database
```
...
- API Port: 9000
- Console Port: 9001
- Credentials: rapidly/rapidlyrapidly
- Data persisted in `minio_data` volume
- Buckets: `rapidly-s3`, `rapidly-s3-public`
...
```bash
# Database
RAPIDLY_POSTGRES_USER=rapidly
RAPIDLY_POSTGRES_PWD=rapidly
RAPIDLY_POSTGRES_DATABASE=rapidly

# MinIO
RAPIDLY_MINIO_USER=rapidly
RAPIDLY_MINIO_PWD=rapidlyrapidly

# S3
RAPIDLY_AWS_ACCESS_KEY_ID=rapidly-development
RAPIDLY_AWS_SECRET_ACCESS_KEY=rapidly123456789
RAPIDLY_S3_FILES_BUCKET_NAME=rapidly-s3
```
