"""Rapidly image-resizer Lambda handler.

Runs as a CloudFront Lambda@Edge origin-request function for the Rapidly
platform.  When a viewer requests an image with ?width= or ?height= query
parameters, this handler checks for a previously cached resize in S3 and,
if absent, fetches the original, resizes it with Pillow, stores the result,
and rewrites the request URI so CloudFront serves the resized variant.
"""
