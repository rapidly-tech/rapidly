---
title: Custom Fields
description: Learn how to add custom input fields to your file-sharing checkout with Rapidly
---

By default, the Checkout form will only ask basic information from the customer to fulfill the purchase: a name, an email address, billing information, etc. But you might need more! A few examples:
...
With Rapidly, you can easily add such fields to your checkout using **Custom Fields**.

## Create Custom Fields

Custom Fields are managed at a workspace's level. To create them, go to **Settings** and **Custom Fields**. You'll see the list of all the available fields on your workspace.
...
The slug determines the key that'll be used to store the data inside objects related to the checkout, like Purchases. It must be unique across your workspace. You can change it afterwards, we'll automatically update the data to reflect the new slug.
...
Custom Fields are enabled on Checkout specifically on each **share**. While [creating or updating](/features/products) a share, you can select the custom fields you want to include in the checkout for this share.
...
The fields are now added as part of the Checkout form for this share.

## Read data

Once you have added Custom Fields to your workspace, they'll be automatically displayed in your dashboard. From there, you'll be able to see the data input by the customer.

This data is also available from the API under the `custom_field_data` property. Each value is referenced by the **slug** of the field.
