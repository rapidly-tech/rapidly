...
Comprehensive load testing suite for Rapidly's payment infrastructure, focusing on checkout flows, webhook processing, and background job performance.
...
- Python 3.14+ with uv
- Running Rapidly API server (or staging environment)
...
```bash
LOAD_TEST_API_TOKEN=Rapidly_oat_...                           # Organization access token (required)
...
LOAD_TEST_API_TOKEN="Rapidly_oat_..." \
uv run task loadtest
