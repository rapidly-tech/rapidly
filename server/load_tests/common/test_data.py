...
def generate_random_email() -> str:
    ...
    return f"loadtest+{random_str}@rapidly.tech"
...
def generate_checkout_data(
    ...
    Args:
        share_id: Product ID to create checkout for (uses config default if None)
    ...
    """
    share_id = share_id or config.share_id
    if not share_id:
...
def generate_event_payload(
    ...
    Note:
        workspace_id is not included in the payload because the load test uses
        workspace tokens (rapidly_oat_*) which infer the workspace from the token.
    """
