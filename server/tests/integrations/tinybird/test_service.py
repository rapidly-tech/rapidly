"""Tests for Tinybird integration service."""
...
from rapidly.integrations.tinybird.client import TinybirdClient
...
# ── Event-to-Tinybird Conversion ──


class TestEventToTinybird:
    def test_basic_conversion(self) -> None:
        event = create_test_event(name="file_share.created")
        result = _event_to_tinybird(event)

        assert result["id"] == str(event.id)
        assert result["name"] == "file_share.created"
        assert result["source"] == "system"
        assert result["organization_id"] == str(event.workspace_id)
        assert result["user_metadata"] == "{}"

    def test_system_event_denormalizes_metadata(self) -> None:
        event = create_test_event(
            name="file_share.payment_completed",
        ...
        # session_id is not a known denormalized field; it stays in user_metadata
...
class TestTinybirdEventsQuery:
    async def test_get_event_type_stats(self, tinybird_client: TinybirdClient) -> None:
        ...
        events = [
            ...
            create_test_event(
                workspace_id=org_id,
                name="file_share.download_completed",
                source=EventSource.system,
            ),
            create_test_event(
                workspace_id=org_id,
        ...
        assert len(stats) == 3
        assert (
            stats_by_name[("file_share.created", EventSource.system)].occurrences == 3
        )
        assert (
            stats_by_name[
                ("file_share.download_completed", EventSource.system)
            ].occurrences
            == 1
        )
    ...
    async def test_filter_by_source(self, tinybird_client: TinybirdClient) -> None:
        ...
        events = [
            create_test_event(
                workspace_id=org_id, name="system.event", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="system.event", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="user.event", source=EventSource.user
            ),
        ]
    ...
    async def test_filter_by_customer_id(self, tinybird_client: TinybirdClient) -> None:
        ...
        events = [
            create_test_event(
                workspace_id=org_id, name="event.a", source=EventSource.user
            ),
            create_test_event(
                workspace_id=org_id, name="event.a", source=EventSource.user
            ),
            create_test_event(
                workspace_id=org_id, name="event.b", source=EventSource.user
            ),
        ]
    ...
    async def test_get_event_type_stats_from_mv(
        ...
        events = [
            create_test_event(workspace_id=org_id, name="page.viewed"),
            create_test_event(workspace_id=org_id, name="page.viewed"),
            create_test_event(workspace_id=org_id, name="page.viewed"),
            create_test_event(workspace_id=org_id, name="page.viewed"),
            create_test_event(workspace_id=org_id, name="button.clicked"),
            create_test_event(workspace_id=org_id, name="button.clicked"),
            create_test_event(workspace_id=org_id, name="form.submitted"),
        ]
        for e in events:
    ...
    async def test_workspace_isolation(self, tinybird_client: TinybirdClient) -> None:
        org_1 = uuid.uuid4()
        org_2 = uuid.uuid4()

        events = [
            create_test_event(
                workspace_id=org_1, name="org1.event", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_1, name="org1.event", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_2, name="org2.event", source=EventSource.system
            ),
        ]
...
class TestTinybirdDelete:
    async def test_delete_by_id(self, tinybird_client: TinybirdClient) -> None:
        ...
        events = [
            create_test_event(
                workspace_id=org_id, name="delete.test", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="delete.test", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="keep.test", source=EventSource.system
            ),
        ]
    ...
    async def test_delete_multiple_by_id(self, tinybird_client: TinybirdClient) -> None:
        ...
        events = [
            create_test_event(
                workspace_id=org_id, name="batch.delete", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="batch.delete", source=EventSource.system
            ),
            create_test_event(
                workspace_id=org_id, name="batch.keep", source=EventSource.system
            ),
        ]
