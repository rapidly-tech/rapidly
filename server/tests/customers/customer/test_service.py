...
from rapidly.core.address import (
    Address,
    AddressInput,
    CountryAlpha2,
    CountryAlpha2Input,
...
from rapidly.customers.customer.types.customer import CustomerCreate, CustomerUpdate
from rapidly.errors import RequestValidationError
from rapidly.identity.auth.models import AuthPrincipal, is_user_principal
from rapidly.identity.member.queries import MemberRepository
from rapidly.models import Customer, User, Workspace, WorkspaceMembership
...
class TestCreate:
    ...
    async def test_not_accessible_workspace(
        ...
        with pytest.raises(RequestValidationError):
            await customer_service.create(
                session,
                CustomerCreate(email="customer@example.com", workspace_id=workspace.id),
                auth_subject,
            )
    ...
    async def test_owner_override_all_fields(
        ...
        assert customer.email == "customer@rapidly.tech"
        ...
        assert member is not None
        assert member.customer_id == customer.id
        assert member.email == "owner@rapidly.tech"
    ...
    async def test_concurrent_create_duplicate_external_id(
        ...
        mocker.patch.object(
            CustomerRepository,
            "get_by_external_id_and_workspace",
            return_value=None,
        )
...
class TestUpdate:
    ...
    async def test_explicit_null_billing_address(
        ...
        # Setting billing_address to None is now allowed (clears the address)
...
@pytest.mark.asyncio
class TestAnonymize:
    async def test_customer_anonymization(
        self,
        session: AsyncSession,
        save_fixture: SaveFixture,
        workspace: Workspace,
    ) -> None:
        """Customer name and email should be hashed."""
        customer = await create_customer(
            save_fixture,
            workspace=workspace,
            email="individual@example.com",
        ...
        anonymized = await customer_service.anonymize_customer(session, customer)

        # Email should be hashed
        assert anonymized.email.endswith("@redacted.invalid")
    ...
    async def test_idempotent(
        ...
        anonymized_again = await customer_service.anonymize_customer(
            session, anonymized
        )
...
class TestWebhook:
    ...
    async def test_scalar_events(
        ...
        send_mock = mocker.patch("rapidly.messaging.webhook.actions.send")
