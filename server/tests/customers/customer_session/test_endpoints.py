"""Tests for customer session endpoints."""
...
from rapidly.models import Customer, Member, Workspace, WorkspaceMembership
...
class TestCreate:
    async def test_anonymous(self, client: AsyncClient) -> None:
        response = await client.post(
            "/api/customer-sessions/", json={"customer_id": str(uuid.uuid4())}
        )
    ...
    async def test_cant_manage_customer(
        ...
        response = await client.post(
            "/api/customer-sessions/", json={"customer_id": str(customer.id)}
        )
