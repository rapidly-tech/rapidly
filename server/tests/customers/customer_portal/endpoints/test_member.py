"""Tests for customer portal member endpoints."""
...
from rapidly.models import Customer, Member, Workspace
...
class TestListMembers:
    async def test_anonymous(self, client: AsyncClient) -> None:
        response = await client.get("/api/customer-portal/members")
...
class TestUpdateMember:
    async def test_anonymous(self, client: AsyncClient) -> None:
        response = await client.patch(
            f"/api/customer-portal/members/{uuid.uuid4()}",
            json={"role": "member"},
        )
        assert response.status_code == 401
    ...
    async def test_cannot_modify_self(
        ...
        response = await client.patch(
            f"/api/customer-portal/members/{member_owner.id}",
            json={"role": "member"},
        )
        assert response.status_code == 422
        assert (
    ...
    async def test_cannot_demote_only_owner(
        ...
        response = await client.patch(
            f"/api/customer-portal/members/{member2.id}",
            json={"role": "member"},
        )
...
class TestRemoveMember:
    async def test_anonymous(self, client: AsyncClient) -> None:
        response = await client.delete(f"/api/customer-portal/members/{uuid.uuid4()}")
        assert response.status_code == 401
    ...
    async def test_cannot_remove_only_owner(
        ...
        response = await client.delete(f"/api/customer-portal/members/{member2.id}")
