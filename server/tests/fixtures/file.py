"""Test helpers for file uploads, S3 multipart workflows, and file fixtures."""
...
class TestFile:
    ...
    async def create(
        self, session: AsyncSession, workspace: Workspace, parts: int = 1
    ) -> FileUpload:
        return await file_service.generate_presigned_upload(
            session,
            workspace=workspace,
            create_schema=self.build_create(workspace.id, parts=parts),
        )

    def build_create(
        self, workspace_id: UUID, parts: int = 1
    ) -> DownloadableFileCreate:
        create_parts = []
    ...
    def validate_create_response(
        self, response: Response, workspace_id: UUID, parts: int = 1
    ) -> FileUpload:
    ...
    async def complete(
        ...
        assert metadata["rapidly-id"] == str(completed.id)
        assert metadata["rapidly-workspace-id"] == str(completed.workspace_id)
        assert metadata["rapidly-name"] == completed.name
        assert metadata["rapidly-size"] == str(self.size)
...
def empty_test_bucket(worker_id: str) -> Iterable[Any]:
    ...
    parsed_s3 = urlparse(settings.S3_ENDPOINT_URL)
    if parsed_s3.hostname != "127.0.0.1":
...
async def uploaded_fixture(
    ...
    created = await file.create(session, workspace)
...
async def uploaded_logo_png(session: AsyncSession, workspace: Workspace) -> File:
    img = TestFile("logo.png")
    return await uploaded_fixture(session, workspace, img)
...
@pytest_asyncio.fixture
async def uploaded_logo_jpg(session: AsyncSession, workspace: Workspace) -> File:
