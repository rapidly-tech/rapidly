"""Builder functions for constructing Stripe API object stubs in tests."""
...
def build_stripe_refund(
    *,
    ...
    if not id:
        id = str(create_uuid()).replace("-", "")
        id = f"re_{id}"

    if not charge_id:
        charge_id = str(create_uuid()).replace("-", "")
