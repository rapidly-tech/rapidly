"""Database lifecycle fixtures — per-worker DB creation, sessions, and model persistence."""
...
from rapidly.core.db.postgres import create_async_engine
...
PersistModel = Callable[[Model], Coroutine[None, None, None]]

# Keep old alias for backwards compatibility in existing tests.
SaveFixture = PersistModel


def _build_persister(session: AsyncSession) -> PersistModel:
    async def _persist(model: Model) -> None:
        session.add(model)
        await session.flush()
...
# Alias used by the majority of tests.
save_fixture_factory = _build_persister


@pytest.fixture
def save_fixture(session: AsyncSession) -> PersistModel:
    return _build_persister(session)
