"""Tests for member session service logic."""
...
from rapidly.identity.member_session.actions import member_session
from rapidly.models import Member, Workspace
from rapidly.models.member import MemberRole
from rapidly.models.member_session import MEMBER_SESSION_TOKEN_PREFIX, MemberSession
...
class TestCreateMemberSession:
    async def test_creates_session_with_token(
        ...
        assert member_session_obj.expires_at > now_utc()
...
class TestGetByToken:
    ...
    async def test_returns_none_for_expired_session(
        ...
        created_session.expires_at = now_utc() - timedelta(hours=1)
    ...
    async def test_eagerly_loads_member_and_customer(
        ...
        assert retrieved_session.member.customer.workspace.id == workspace.id
