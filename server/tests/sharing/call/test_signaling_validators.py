"""Auth-validator tests for ``session_kind="call"``."""
...
class TestCallHostValidator:
    async def test_valid_host_secret_passes(self, redis: Any) -> None:
        ...
        assert await _AUTH_VALIDATORS[("call", "host")](ctx) is True
    ...
    async def test_wrong_host_secret_fails_4003(self, redis: Any) -> None:
        ...
        assert await _AUTH_VALIDATORS[("call", "host")](ctx) is False
...
class TestCallGuestValidator:
    async def test_valid_invite_token_passes(self, redis: Any) -> None:
        ...
        assert await _AUTH_VALIDATORS[("call", "guest")](ctx) is True
    ...
    async def test_unknown_invite_fails_4003(self, redis: Any) -> None:
        ...
        assert await _AUTH_VALIDATORS[("call", "guest")](ctx) is False
        assert ws.close_code == 4003
