"""Tests for Watch chamber's ``InviteTokenRepository`` — the Redis SET
that backs watch-together invite-token validation.

Mirrors the collab + screen chamber tests. The only intended difference
is the Redis key prefix (``file-sharing:watch:invite:``).
"""
...
from rapidly.sharing.watch.queries import InviteTokenRepository
...
def _invite_key(slug: str) -> str:
    return f"file-sharing:watch:invite:{slug}"
