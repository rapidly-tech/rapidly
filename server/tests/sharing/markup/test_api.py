"""HTTP tests for the Collab chamber endpoints (``/api/v1/markup/*``)."""
...
class TestCreateCollabSessionEndpoint:
    async def test_returns_payload_with_slugs_and_secret(
        ...
        assert body["secret"]
        assert body["invite_template"].startswith("/markup/")

    async def test_accepts_minimum_body(self, client: AsyncClient) -> None:
        response = await client.post("/api/v1/markup/session", json={})
        assert response.status_code == 200

    async def test_rejects_max_participants_over_cap(self, client: AsyncClient) -> None:
        response = await client.post(
            "/api/v1/markup/session", json={"max_participants": 9}
        )
    ...
    async def test_rejects_max_participants_below_two(
        ...
        response = await client.post(
            "/api/v1/markup/session", json={"max_participants": 1}
        )
        assert response.status_code == 422

    async def test_rejects_invalid_kind(self, client: AsyncClient) -> None:
        response = await client.post(
            "/api/v1/markup/session", json={"kind": "spreadsheet"}
        )
        assert response.status_code == 422
...
class TestMintInviteEndpoint:
    async def test_returns_token_for_valid_secret(self, client: AsyncClient) -> None:
        created = (
            await client.post("/api/v1/markup/session", json={"max_participants": 4})
        ).json()
        response = await client.post(
            f"/api/v1/markup/session/{created['short_slug']}/invite",
            json={"secret": created["secret"]},
        )
        assert response.status_code == 200
        body = response.json()
        assert body["invite_token"]
        assert body["invite_url"].startswith(f"/markup/{created['short_slug']}")
...
class TestGetCollabSessionEndpoint:
    async def test_returns_public_view_without_secrets(
        ...
        response = await client.get(f"/api/v1/markup/session/{created['short_slug']}")
        assert response.status_code == 200
        body = response.json()
    ...
    async def test_returns_404_for_unknown_session(self, client: AsyncClient) -> None:
        response = await client.get("/api/v1/markup/session/missing")
        assert response.status_code == 404
...
class TestCloseCollabSessionEndpoint:
    async def test_returns_204_on_success(self, client: AsyncClient) -> None:
        created = (
            await client.post("/api/v1/markup/session", json={"max_participants": 3})
        ).json()
        response = await client.request(
            "DELETE",
            f"/api/v1/markup/session/{created['short_slug']}",
            json={"secret": created["secret"]},
        )
