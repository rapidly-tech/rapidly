...
from rapidly.sharing.file_sharing.signaling_transport import (
    RegisterResult,
    RoomTransport,
)
...
class TestRegisterPeer:
    """register_peer consolidates the three admission checks (PR 4b)."""

    async def test_first_host_gets_ok_and_sets_host_id(self) -> None:
        mgr = SignalingManager()
        peer = Peer(peer_id="host-1", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        result = await mgr.register_peer("room-1", peer)
        assert result is RegisterResult.OK
        room = mgr.get_room("room-1")
        assert room is not None
        assert room.host_id == "host-1"
        assert "host-1" in room.peers

    async def test_second_host_registration_fails(self) -> None:
        mgr = SignalingManager()
        first = Peer(peer_id="host-1", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        second = Peer(peer_id="host-2", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        assert await mgr.register_peer("room-1", first) is RegisterResult.OK
        assert await mgr.register_peer("room-1", second) is RegisterResult.HOST_TAKEN
        room = mgr.get_room("room-1")
        assert room is not None
        assert room.host_id == "host-1"  # original host retained
        assert "host-2" not in room.peers  # rejected peer not added

    async def test_guest_registration_is_ok(self) -> None:
        mgr = SignalingManager()
        host = Peer(peer_id="host-1", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        guest = Peer(peer_id="guest-1", ws=_MockWs(), role="guest")  # type: ignore[arg-type]
        assert await mgr.register_peer("room-1", host) is RegisterResult.OK
        assert await mgr.register_peer("room-1", guest) is RegisterResult.OK
        room = mgr.get_room("room-1")
        assert room is not None
        assert room.host_id == "host-1"  # guest did not clobber host
        assert "guest-1" in room.peers

    async def test_room_full_rejects_further_peers(self) -> None:
        mgr = SignalingManager()
        # Fill the room to capacity with a host + (cap - 1) guests.
        host = Peer(peer_id="host-1", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        assert await mgr.register_peer("room-1", host) is RegisterResult.OK
        for i in range(MAX_PEERS_PER_ROOM - 1):
            ...
            assert await mgr.register_peer("room-1", guest) is RegisterResult.OK
        overflow = Peer(peer_id="overflow", ws=_MockWs(), role="guest")  # type: ignore[arg-type]
        assert await mgr.register_peer("room-1", overflow) is RegisterResult.ROOM_FULL
        room = mgr.get_room("room-1")
        assert room is not None
        assert "overflow" not in room.peers

    async def test_failed_host_admission_cleans_up_empty_room(self) -> None:
        """If register_peer fails on an otherwise-empty room it just
        created, the empty room must not linger."""
        mgr = SignalingManager()
        # Seed: first host lands, OK.
        host = Peer(peer_id="host-1", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        await mgr.register_peer("room-1", host)
        # Now the host disconnects, leaving the room empty but present.
        mgr.remove_peer("room-1", "host-1")
        room = mgr.get_room("room-1")
        assert room is not None  # still there, just empty
        # A new host comes in and fills it — this should succeed, proving
        # the empty room is reusable.
        new_host = Peer(peer_id="host-2", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        assert await mgr.register_peer("room-1", new_host) is RegisterResult.OK


@pytest.mark.asyncio
class TestPeerExists:
    async def test_true_for_registered_peer(self) -> None:
        mgr = SignalingManager()
        peer = Peer(peer_id="p", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        await mgr.register_peer("r", peer)
        assert await mgr.peer_exists("r", "p") is True

    async def test_false_for_unknown_peer(self) -> None:
        mgr = SignalingManager()
        peer = Peer(peer_id="p", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        await mgr.register_peer("r", peer)
        assert await mgr.peer_exists("r", "ghost") is False

    async def test_false_for_unknown_room(self) -> None:
        mgr = SignalingManager()
        assert await mgr.peer_exists("no-such-room", "anyone") is False


@pytest.mark.asyncio
class TestHostIdFor:
    async def test_returns_host_peer_id(self) -> None:
        mgr = SignalingManager()
        peer = Peer(peer_id="h", ws=_MockWs(), role="host")  # type: ignore[arg-type]
        await mgr.register_peer("r", peer)
        assert await mgr.host_id_for("r") == "h"

    async def test_returns_none_before_host_registers(self) -> None:
        mgr = SignalingManager()
        mgr.get_or_create_room("r")  # room exists but no host yet
        assert await mgr.host_id_for("r") is None

    async def test_returns_none_for_unknown_room(self) -> None:
        mgr = SignalingManager()
        assert await mgr.host_id_for("no-such-room") is None
