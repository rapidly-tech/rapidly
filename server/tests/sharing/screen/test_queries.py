"""Tests for Screen chamber's ``InviteTokenRepository`` — the Redis SET
that backs screen-share invite-token validation.

Mirrors the collab chamber test from Phase 66. The only intended
difference is the Redis key prefix (``file-sharing:screen:invite:``).
"""
...
from rapidly.sharing.screen.queries import InviteTokenRepository
...
def _invite_key(slug: str) -> str:
    return f"file-sharing:screen:invite:{slug}"
