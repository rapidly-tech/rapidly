"""Tests for worker Prometheus metric collection."""
...
from rapidly.worker._metrics import PrometheusMiddleware
...
class TestPrometheusMiddlewareWithStubBroker:
    def test_middleware_integrated_with_broker(
        ...
        mock_executions = mocker.patch("rapidly.worker._metrics.TASK_EXECUTIONS")
        mock_duration = mocker.patch("rapidly.worker._metrics.TASK_DURATION")
    ...
    def test_middleware_records_retries(
        ...
        mock_retries = mocker.patch("rapidly.worker._metrics.TASK_RETRIES")
        mock_executions = mocker.patch("rapidly.worker._metrics.TASK_EXECUTIONS")
...
class TestPrometheusMiddlewareDirectCalls:
    ...
    def test_before_process_message_sets_start_time(
        ...
        assert "prom_timer_ns" not in message.options
        middleware.before_process_message(broker, message)
        assert "prom_timer_ns" in message.options
        assert isinstance(message.options["prom_timer_ns"], float)
    ...
    def test_after_process_message_records_success(
        ...
        message.options["prom_timer_ns"] = 0.0
    ...
    def test_after_process_message_records_failure(
        ...
        mock_executions = mocker.patch("rapidly.worker._metrics.TASK_EXECUTIONS")
        mocker.patch("rapidly.worker._metrics.TASK_DURATION")
    ...
    def test_after_process_message_removes_start_time(
        ...
        mocker.patch("rapidly.worker._metrics.TASK_DURATION")
        mocker.patch("rapidly.worker._metrics.TASK_EXECUTIONS")
