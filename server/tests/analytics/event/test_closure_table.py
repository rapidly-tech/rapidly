"""Tests for event closure table population and hierarchy."""

import pytest
from sqlalchemy import func, select

from rapidly.analytics.event import actions as event_service
from rapidly.analytics.event.types import EventCreateExternalCustomer, EventsIngest
from rapidly.core.db.postgres import AsyncSession
from rapidly.identity.auth.models import AuthPrincipal
from rapidly.models import Event, EventClosure, Workspace
...
class TestClosureTable:
    async def test_closure_table_population_root_event(
        ...
        workspace = await create_workspace(save_fixture)
    ...
    async def test_closure_table_batch_ingest_out_of_order(
        ...
        customer = await create_customer(
            save_fixture, workspace=workspace, external_id="test-customer-123"
        )
        ...
        id_result = await session.execute(
            select(Event.id).where(Event.workspace_id == workspace.id)
        )
        ...
        event_result = await session.execute(
            select(Event).where(Event.workspace_id == workspace.id).order_by(Event.name)
        )
