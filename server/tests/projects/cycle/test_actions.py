...
class TestArchiveDelete:
    ...
    async def test_unarchive_requires_admin(
        ...
        # Pin: unarchive is a state-restoration operation symmetric to
        # archive — it must require the same admin floor, otherwise a
        # plain member could un-archive cycles other admins shut down.
        principal = _user_principal()
        cycle = _cycle()
        session = MagicMock()
