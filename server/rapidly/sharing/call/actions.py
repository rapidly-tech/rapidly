"""Business logic for the Call chamber."""
...
# v1 default: 1 hour. Calls typically don't need longer; hosts can
# recreate for longer sessions.
CALL_SESSION_TTL = 3600
...
async def create_call_session(
    ...
    *,
    ...
    """Create a new call session. Returns (channel, raw_secret)."""
    repo = ChannelRepository(redis)
    now_iso = datetime.now(UTC).isoformat()
    channel, raw_secret = await repo.create_channel(
        max_downloads=0,
        ttl=CALL_SESSION_TTL,
        title=title,
        user_id=user_id,
    )
    channel.session_kind = "call"
    channel.call_mode = mode
    channel.max_participants = max_participants
    channel.call_started_at = now_iso
...
async def mint_invite_token(redis: Redis, slug: str, secret: str) -> str | None:
    """Mint an invite token for a call session.

    Refuses to mint for non-call channels so a mis-routed slug can't
    silently cross chambers (same security invariant as Screen/Watch).
    """
    channel_repo = ChannelRepository(redis)
    channel = await channel_repo.fetch_channel(slug)
    if channel is None:
        return None
    if channel.session_kind != "call":
        return None
...
async def validate_invite_token(redis: Redis, slug: str, token: str) -> bool:
    """Called from the ``("call", "guest")`` signaling auth validator."""
...
async def get_public_view(redis: Redis, slug: str) -> dict[str, object] | None:
    ...
    if channel is None or channel.session_kind != "call":
        return None
    return {
        "short_slug": channel.short_slug,
        "title": channel.title,
        "max_participants": channel.max_participants,
        "mode": channel.call_mode,
        "started_at": channel.call_started_at,
        "host_connected": channel.call_started_at is not None,
    }


async def close_call_session(redis: Redis, slug: str, secret: str) -> bool:
    channel_repo = ChannelRepository(redis)
    channel = await channel_repo.fetch_channel(slug)
    if channel is None or channel.session_kind != "call":
        return False
    ...
    _log.info("call_session_closed", short_slug=channel.short_slug)
    return True


def is_enabled() -> bool:
    return settings.FILE_SHARING_CALL_ENABLED


def session_expires_at() -> str:
    return (datetime.now(UTC) + timedelta(seconds=CALL_SESSION_TTL)).isoformat()
