"""HTTP endpoints for the Markup chamber (``/api/v1/markup/*``).

Originally the Collab chamber; renamed in M1.4 of the engineering-
suite migration. Internal identifiers (``CollabSession*``, ``collab_service``,
Redis key namespace ``file-sharing:collab:*``) still use the historical
``collab`` prefix for compatibility — kept until a v2 cleanup PR
renames them. Feature-flagged by ``FILE_SHARING_COLLAB_ENABLED``.
"""
...
router = APIRouter(prefix="/v1/markup", tags=["markup", APITag.private])
...
async def create_session(
    ...
    return CreateCollabSessionResponse(
        ...
        secret=raw_secret,
        invite_template=f"/markup/{channel.short_slug}?t={{token}}",
...
async def mint_invite(
    ...
    return MintInviteResponse(
        invite_token=token,
        invite_url=f"/markup/{slug}?t={token}",
    )
