...
async def fetch_channel(
    ...
    return ChannelFetchResponse(
        ...
        creator_country=channel.creator_country,
    )
...
async def build_ice_config() -> ICEConfigResponse:
    ...
    if settings.FILE_SHARING_COTURN_ENABLED:
        # Generate ephemeral TURN credentials using HMAC-SHA256.
        # Protocol: username = "expiry_timestamp:random_label"
        #           credential = Base64(HMAC-SHA256(shared_secret, username))
        # COTURN validates these natively with use-auth-secret + sha256
        # (requires coturn ≥ 4.6 with --sha256 flag).
