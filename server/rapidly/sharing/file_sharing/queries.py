...
class ChannelRepository:
    ...
    async def store_checkout_payment_token(
        ...
        await self._redis.setex(key, self._CHECKOUT_TOKEN_TTL, payment_token)
    ...
    async def claim_checkout_payment_token(
        ...
        Returns the token if found, None otherwise. One-time use — the key
        is deleted after retrieval to prevent replay.
        """
        key = self._checkout_token_key(slug, checkout_session_id)
        # GETDEL is atomic — prevents race conditions
        token: str | None = await self._redis.getdel(key)
        return token
...
class SecretRepository:
    ...
    async def store_secret_checkout_payment_token(
        ...
        await self._redis.setex(key, self._SECRET_CHECKOUT_TOKEN_TTL, payment_token)
