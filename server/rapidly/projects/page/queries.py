"""Page persistence layer."""

from collections.abc import Sequence
from typing import Any
from uuid import UUID

from sqlalchemy import Select, asc, desc, select
from sqlalchemy.sql.elements import UnaryExpression

from rapidly.core.ordering import Sorting
from rapidly.core.queries import FindByIdMixin, Repository, SoftDeleteMixin
from rapidly.identity.auth.models import (
    AuthPrincipal,
    User,
    Workspace,
    is_user_principal,
    is_workspace_principal,
)
from rapidly.models import (
    Project,
    ProjectPage,
    ProjectPageAccess,
    WorkspaceMembership,
)
from rapidly.projects.page.ordering import ProjectPageSortProperty


class ProjectPageRepository(
    SoftDeleteMixin[ProjectPage],
    FindByIdMixin[ProjectPage, UUID],
    Repository[ProjectPage],
):
    model = ProjectPage

    def get_readable_statement(
        self, auth_subject: AuthPrincipal[User | Workspace]
    ) -> Select[tuple[ProjectPage]]:
        statement = (
            self.get_base_statement()
            .join(Project, Project.id == ProjectPage.project_id)
            # Soft-deleted projects' children must never surface.
            .where(Project.deleted_at.is_(None))
        )

        if is_user_principal(auth_subject):
            user = auth_subject.subject
            statement = statement.where(
                Project.workspace_id.in_(
                    select(WorkspaceMembership.workspace_id).where(
                        WorkspaceMembership.user_id == user.id,
                        WorkspaceMembership.deleted_at.is_(None),
                    )
                ),
                # Private pages are visible only to the owner.  Workspace
                # tokens see everything in their own workspace by the
                # workspace-principal branch below.
                (ProjectPage.access == ProjectPageAccess.public)
                | (ProjectPage.owner_id == user.id),
            )
        elif is_workspace_principal(auth_subject):
            statement = statement.where(Project.workspace_id == auth_subject.subject.id)

        return statement

    def apply_sorting(
        self,
        statement: Select[tuple[ProjectPage]],
        sorting: Sequence[Sorting[ProjectPageSortProperty]],
    ) -> Select[tuple[ProjectPage]]:
        clauses: list[UnaryExpression[Any]] = []
        for criterion, is_desc in sorting:
            fn = desc if is_desc else asc
            column = getattr(ProjectPage, criterion.value)
            clauses.append(fn(column))
        return statement.order_by(*clauses) if clauses else statement

    async def get_by_slug(self, project_id: UUID, slug: str) -> ProjectPage | None:
        statement = self.get_base_statement().where(
            ProjectPage.project_id == project_id,
            ProjectPage.slug == slug,
        )
        return await self.get_one_or_none(statement)
