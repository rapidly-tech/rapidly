"""HTTP routes for project saved views."""
...
async def list(
    ...
    include_archived: bool = Query(
        False, description="Include archived views in the result."
    ),
    ...
    return PaginatedList.from_paginated_results(
        [schemas.ProjectView.model_validate(v) for v in results],
        count,
        pagination,
    )
...
async def archive(
    ...
    if view is None:
        raise ResourceNotFound()
    updated = await view_actions.archive(session, auth_subject, view)
    return schemas.ProjectView.model_validate(updated)
...
async def unarchive(
    ...
    if view is None:
        raise ResourceNotFound()
    updated = await view_actions.unarchive(session, auth_subject, view)
    return schemas.ProjectView.model_validate(updated)
