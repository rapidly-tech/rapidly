"""Work-item external link persistence layer."""
...
from rapidly.models import Project, WorkItem, WorkItemLink, WorkspaceMembership
from rapidly.projects.external_link.ordering import WorkItemLinkSortProperty


class WorkItemLinkRepository(
    SoftDeleteMixin[WorkItemLink],
    FindByIdMixin[WorkItemLink, UUID],
    Repository[WorkItemLink],
):
    model = WorkItemLink

    def get_readable_statement(
        self, auth_subject: AuthPrincipal[User | Workspace]
    ) -> Select[tuple[WorkItemLink]]:
        statement = (
            self.get_base_statement()
            .join(WorkItem, WorkItem.id == WorkItemLink.work_item_id)
