"""Background jobs for customer lifecycle: state sync, event emission, cleanup.

Handles deferred operations like emitting ``customer.created`` system
events, syncing customer state after payment changes, and cascading
soft-deletes to related records.
"""
...
from rapidly.analytics.event import actions as event_service
...
from rapidly.models.webhook_endpoint import CustomerWebhookEventType
from rapidly.worker import AsyncSessionMaker, RedisMiddleware, TaskPriority, actor

from . import actions as customer_service
from .queries import CustomerRepository


class CustomerTaskError(BackgroundTaskError): ...
