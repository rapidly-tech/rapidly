"""Stripe Connect account Pydantic models.

Defines the ``Account`` read model with billing details, the
``AccountCreateForWorkspace`` input, ``AccountUpdate`` for mutable
billing fields, and ``AccountLink`` for Stripe-hosted onboarding URLs.
"""

from enum import StrEnum
from typing import Annotated, Literal
from uuid import UUID

from pydantic import BeforeValidator, Field

from rapidly.core.address import Address, AddressInput
from rapidly.core.types import Schema
from rapidly.enums import AccountType
from rapidly.models.account import Account as AccountModel
from rapidly.platform.user.types import UserBase
from rapidly.platform.workspace.types import Workspace

# ---------------------------------------------------------------------------
# Stripe-supported countries
# ---------------------------------------------------------------------------


class StripeAccountCountry(StrEnum):
    """ISO 3166-1 alpha-2 codes for Stripe-supported account countries."""

    AL = "AL"
    AG = "AG"
    AR = "AR"
    AM = "AM"
    AU = "AU"
    AT = "AT"
    BH = "BH"
    BE = "BE"
    BO = "BO"
    BA = "BA"
    BG = "BG"
    KH = "KH"
    CA = "CA"
    CL = "CL"
    CO = "CO"
    CR = "CR"
    HR = "HR"
    CY = "CY"
    CZ = "CZ"
    CI = "CI"
    DK = "DK"
    DO = "DO"
    EC = "EC"
    EG = "EG"
    SV = "SV"
    EE = "EE"
    ET = "ET"
    FI = "FI"
    FR = "FR"
    GM = "GM"
    DE = "DE"
    GH = "GH"
    GR = "GR"
    GT = "GT"
    GY = "GY"
    HK = "HK"
    HU = "HU"
    IS = "IS"
    IN = "IN"
    ID = "ID"
    IE = "IE"
    IL = "IL"
    IT = "IT"
    JM = "JM"
    JP = "JP"
    JO = "JO"
    KE = "KE"
    KW = "KW"
    LV = "LV"
    LI = "LI"
    LT = "LT"
    LU = "LU"
    MO = "MO"
    MG = "MG"
    MY = "MY"
    MT = "MT"
    MU = "MU"
    MX = "MX"
    MD = "MD"
    MN = "MN"
    MA = "MA"
    NA = "NA"
    NL = "NL"
    NZ = "NZ"
    NG = "NG"
    MK = "MK"
    NO = "NO"
    OM = "OM"
    PA = "PA"
    PY = "PY"
    PE = "PE"
    PH = "PH"
    PL = "PL"
    PT = "PT"
    QA = "QA"
    RO = "RO"
    RW = "RW"
    SA = "SA"
    SN = "SN"
    RS = "RS"
    SG = "SG"
    SK = "SK"
    SI = "SI"
    ZA = "ZA"
    KR = "KR"
    ES = "ES"
    LK = "LK"
    LC = "LC"
    SE = "SE"
    CH = "CH"
    TZ = "TZ"
    TH = "TH"
    TT = "TT"
    TN = "TN"
    TR = "TR"
    AE = "AE"
    GB = "GB"
    US = "US"
    UY = "UY"
    UZ = "UZ"
    VN = "VN"
    DZ = "DZ"
    AO = "AO"
    AZ = "AZ"
    BS = "BS"
    BD = "BD"
    BJ = "BJ"
    BT = "BT"
    BW = "BW"
    BN = "BN"
    GA = "GA"
    KZ = "KZ"
    LA = "LA"
    MC = "MC"
    MZ = "MZ"
    NE = "NE"
    PK = "PK"
    SM = "SM"
    TW = "TW"


# ---------------------------------------------------------------------------
# Mutation payloads
# ---------------------------------------------------------------------------


class AccountCreateForWorkspace(Schema):
    """Create or retrieve a Stripe Connect account for a workspace."""

    workspace_id: UUID = Field(
        description="Workspace to create or retrieve a payout account for"
    )
    account_type: Literal[AccountType.stripe]
    country: Annotated[StripeAccountCountry, BeforeValidator(str.upper)]


class AccountUpdate(Schema):
    """Mutable billing fields on a payout account."""

    billing_name: str | None = None
    billing_address: AddressInput | None = None
    billing_additional_info: str | None = None
    billing_notes: str | None = None


# ---------------------------------------------------------------------------
# Read model
# ---------------------------------------------------------------------------


class Account(Schema):
    """Full account representation returned by the API."""

    id: UUID
    account_type: AccountType
    status: AccountModel.Status
    stripe_id: str | None
    is_details_submitted: bool
    is_charges_enabled: bool
    is_payouts_enabled: bool
    country: str
    credit_balance: int

    billing_name: str | None
    billing_address: Address | None
    billing_additional_info: str | None
    billing_notes: str | None

    users: list[UserBase]
    workspaces: list[Workspace]


# ---------------------------------------------------------------------------
# Onboarding link
# ---------------------------------------------------------------------------


class AccountLink(Schema):
    """Stripe-hosted onboarding or dashboard URL."""

    url: str
