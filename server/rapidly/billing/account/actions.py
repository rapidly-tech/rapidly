...
from rapidly.errors import ErrorCode, RapidlyError
...
class AccountServiceError(RapidlyError):
    ...
    Defaults to HTTP 400 because these errors represent a user-recoverable
    problem with the account request — typically a Stripe rejection that the
    operator can fix (e.g. completing the Connect platform profile, picking a
    supported country). They should not page on-call as 500s.
    """
