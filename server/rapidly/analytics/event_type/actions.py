"""Event-type CRUD and schema-validation service.

Manages the catalogue of named event types within an workspace,
including JSON-schema validation of event metadata, label resolution,
and property-selector configuration for dynamic event labels.
"""
...
from rapidly.models import Event, EventType, User, Workspace
...
async def list_with_stats(
    ...
    *,
    ...
    db_results, db_count = await _list_with_stats_from_db(
        session,
        auth_subject,
    ...
    org = await _get_tinybird_enabled_org(session, auth_subject, workspace_id)
    if org is None:
    ...
    try:
        (
            tinybird_results,
            tinybird_count,
        ) = await _list_with_stats_from_tinybird(
            session,
            org,
            customer_id=customer_id,
            external_customer_id=external_customer_id,
    ...
    if tinybird_shadow:
        _log_comparison(org.id, db_results, db_count, tinybird_results, tinybird_count)
        return db_results, db_count
...
async def _get_tinybird_enabled_org(
    ...
    if is_workspace_principal(auth_subject):
        ...
        org = await workspace_repository.get_accessible_by_id(
            workspace_id[0], auth_subject.subject.id
        )
        if org is None:
            return None
    else:
...
async def _list_with_stats_from_db(
    ...
    *,
    ...
    statement = event_type_repository.get_event_types_with_stats_statement(auth_subject)

    if workspace_id is not None:
        statement = statement.where(EventType.workspace_id.in_(workspace_id))
    ...
    if query is not None:
        # See [[escape-like-needs-escape-clause]]: ``escape_like``'s
        # backslash prefixes are inert unless ``escape="\\"`` is passed.
    ...
    return _build_event_types_with_stats(results), count
...
async def _list_with_stats_from_tinybird(
    ...
    *,
    ...
    if requires_raw_table:
        tinybird_query = TinybirdEventsQuery(workspace.id)
        if customer_id is not None:
        ...
        tinybird_query = TinybirdEventTypesQuery(workspace.id)
        if source is not None:
            tinybird_query.filter_source(source)
    ...
    event_type_repository = EventTypeRepository.from_session(session)
    event_types_by_name = await event_type_repository.get_by_names_and_workspace(
        names, workspace.id
    )
    ...
    results = _build_event_types_from_tinybird(
        paginated_stats, event_types_by_name, workspace.id
    )
...
def _build_event_types_with_stats(
    results: Sequence[tuple[EventType, EventSource, int, Any, Any]],
...
async def update(
    ...
    repository = EventTypeRepository.from_session(session)
    return await repository.update_label(event_type, label, label_property_selector)
