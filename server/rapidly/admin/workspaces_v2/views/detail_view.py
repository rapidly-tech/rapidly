"""Workspace detail view with horizontal tabs and sidebar."""
...
class WorkspaceDetailView:
    """Render the workspace detail view with horizontal section tabs."""
    ...
    def section_tabs(
        ...
        tabs = [
            Tab(
                "Overview",
                str(request.url_for("workspaces-v2:detail", workspace_id=self.org.id))
                + "?section=overview",
        ...
        with tag.aside(classes="w-80 pl-4"):
            ...
            if self.org.internal_notes:
                ...
                with card(bordered=True, classes="mb-4"):
                    ...
                    with button(
                        ...
                        hx_get=str(
                            request.url_for(
                                "workspaces-v2:add_note", workspace_id=self.org.id
                            )
                        ),
                        hx_target="#modal",
                    ):
            ...
            with card(bordered=True, classes="mb-4"):
                ...
                with tag.div(classes="space-y-2"):
                    ...
                    if is_blocked:
                        # Blocked workspaces can be unblocked and approved
                    ...
                    elif self.org.status == WorkspaceStatus.DENIED:
                        # Denied workspaces can be approved
                        with tag.div(classes="w-full"):
                            with button(
                                variant="secondary",
                                size="sm",
                    ...
                    elif self.org.status == WorkspaceStatus.ACTIVE:
                        # Active workspaces can be denied
                        with tag.div(classes="w-full"):
                            with button(
                                variant="secondary",
                                size="sm",
                        ...
                        with tag.div(classes="w-full"):
                            with button(
                                ...
                                hx_post=str(
                                    request.url_for(
                                        "workspaces-v2:approve",
                                        workspace_id=self.org.id,
                                    )
                                )
                                + f"?threshold={next_threshold}",
                                hx_confirm=f"Approve this workspace with {next_threshold_display} threshold?",
                            ):
                        ...
                        approve_url = str(
                            request.url_for(
                                "workspaces-v2:approve", workspace_id=self.org.id
                            )
                        )
    ...
    def render(self, request: Request, section: str = "overview") -> Generator[None]:
        ...
        with tag.div(classes="mb-6"):
            with tag.a(
                href=str(request.url_for("workspaces-v2:list")),
