"""Admin panel user management routes (HTMX).

Server-rendered admin views for listing users, viewing user details,
managing OAuth connections, and triggering impersonation sessions.
"""
...
from rapidly.models.user import IdentityVerificationStatus, OAuthAccount
from rapidly.platform.user import actions as user_service
from rapidly.platform.user import ordering
...
from .queries import AdminUserRepository
...
async def get(
    ...
    with layout(
        ...
        ### Workspaces ###
        #####################
        orgs = await WorkspaceRepository.from_session(session).get_all(
            WorkspaceRepository.from_session(session)
        ...
        with tag.div(classes="flex flex-col gap-4 pt-16"):
            ...
            with datatable.Datatable[Workspace, WorkspaceSortProperty](
                ...
                datatable.DatatableActionsColumn(
                    "",
                    datatable.DatatableActionHTMX[Workspace](
                        "Delete Workspace",
                        lambda r, i: str(r.url_for("workspaces:delete", id=i.id)),
                        target="#modal",
        ...
        ### OAuth Accounts  ###
        ########################
        admin_user_repo = AdminUserRepository.from_session(session)
        active_oauth_accounts = await admin_user_repo.get_active_oauth_accounts(user.id)
        deleted_oauth_accounts = await admin_user_repo.get_deleted_oauth_accounts(
            user.id
        )
