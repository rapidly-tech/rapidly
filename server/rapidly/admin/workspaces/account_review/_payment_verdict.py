"""Admin panel payment-verdict rendering component for account review.

Renders the Stripe payment-capability assessment within the
workspace review workflow, displaying verification status and
required actions.
"""
...
from ..forms import ApproveWorkspaceForm, UnderReviewWorkspaceForm
            ...
            with tag.div(classes="mt-4 pt-4 border-t border-gray-200"):
                with tag.div(classes="space-y-2"):
                    ...
                    if self.workspace:
                        with self._render_metric_row(
                            "Next Review Threshold",
                            self._format_currency(self.workspace.next_review_threshold),
                        ):
                            pass

                        # Review Actions (context-sensitive based on workspace status)
                        if self.show_actions and self.request:
                            with tag.div(classes="mt-3 pt-3 border-t border-gray-200"):
                                if self.workspace.is_under_review:
                                    ...
                                    with ApproveWorkspaceForm.render(
                                        data=None,
                                        method="POST",
                                    ...
                                    with UnderReviewWorkspaceForm.render(
                                        data=None,
                                        method="POST",
                                        action=str(self.request.url),
                                        classes="space-y-4",
