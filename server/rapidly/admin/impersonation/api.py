...
from ..responses import HXRedirectResponse, validate_redirect_url
...
# Session tokens are URL-safe base64 characters only.
...
def _validated_cookie_value(value: str) -> str:
    """Reject cookie values containing control characters or unexpected content."""
    if not _SAFE_COOKIE_RE.match(value):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid session cookie value",
        )
    return value
...
def _set_session_cookie(
    ...
    # Sanitize: strip any control characters to prevent cookie header injection.
    sanitized = re.sub(r"[\x00-\x1f\x7f]", "", value)
    response.set_cookie(
        key, value=sanitized, expires=expires, httponly=httponly, **_COOKIE_DEFAULTS
    )
...
async def start_impersonation(
    ...
    if (
        ...
        _set_session_cookie(
            ...
            _validated_cookie_value(current_token),
...
async def end_impersonation(
    ...
    raw_admin_token = request.cookies.get(settings.IMPERSONATION_COOKIE_KEY)
    if not raw_admin_token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No admin session found"
        )
    admin_token = _validated_cookie_value(raw_admin_token)
    ...
    response = RedirectResponse(validate_redirect_url(redirect_target))
