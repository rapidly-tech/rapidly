...
def set_login_cookie(
    ...
    *,
    ...
    For cross-site flows (e.g. Apple Sign In POST callback), the cookie
    is skipped entirely because SameSite=None weakens CSRF protection.
    The Redis-backed state parameter provides sufficient verification.
    """
    if cross_site:
        return
    response.set_cookie(
...
async def validate_callback(
    ...
    *,
    ...
    For cross-site flows the cookie was never set, so only the
    Redis-backed state parameter is verified.
    """
