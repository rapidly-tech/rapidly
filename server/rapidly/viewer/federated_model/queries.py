"""Federated-model persistence layer."""
...
from rapidly.models import FederatedModel, Project, WorkspaceMembership


class FederatedModelRepository(
    SoftDeleteMixin[FederatedModel],
    FindByIdMixin[FederatedModel, UUID],
    Repository[FederatedModel],
):
    model = FederatedModel

    def get_readable_statement(
        self, auth_subject: AuthPrincipal[User | Workspace]
    ) -> Select[tuple[FederatedModel]]:
        statement = self.get_base_statement().join(
            Project, Project.id == FederatedModel.project_id
        )
