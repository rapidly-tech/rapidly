"""Low-level Tinybird / ClickHouse HTTP client.

Wraps ``clickhouse-connect`` and raw HTTP calls to push NDJSON event
batches and execute parameterised ClickHouse SQL queries against the
Tinybird workspace.
"""
...
tracer = trace.get_tracer("rapidly.integrations.tinybird")
...
class TinybirdClient:
    ...
    async def delete(self, datasource: str, delete_condition: str) -> dict[str, Any]:
        """Delete rows from a datasource matching the condition."""
        with logfire.span(
