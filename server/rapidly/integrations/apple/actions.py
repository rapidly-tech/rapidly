"""Apple Sign-In integration: client-secret generation and user-info parsing.

Generates the ephemeral JWT client secret required by Apple's OAuth2
endpoint (signed with the team's ES256 key) and extracts user profile
data from the identity token returned during the callback.
"""
...
class AppleServiceError(RapidlyError): ...
...
class AccountLinkedToAnotherUserError(AppleServiceError):
    def __init__(self) -> None:
        message = (
            "This Apple account is already linked to another user on Rapidly. "
...
async def get_updated_or_create(
    ...
    *,
    ...
    profile = await _decode_profile(token["id_token"])
    ...
    if user is not None:
        ...
        if oauth_account is None:
            raise ValueError("Expected Apple OAuth account for existing user")
...
async def _decode_profile(id_token: str) -> AppleUserProfile:
