...
locals {
  ...
  read_replica = [for r in render_postgres.db.read_replicas : r if r.name == "rapidly-read"][0]
...
resource "render_project" "rapidly" {
  name = "Rapidly"
  environments = {
    "Production" : {
...
resource "render_postgres" "db" {
  environment_id = render_project.rapidly.environments["Production"].id
  name           = "db"
  database_name  = "rapidly_cpit"
  database_user  = "rapidly_cpit_user"
  plan           = "pro_64gb"
  region         = "ohio"
  version        = "15"
  disk_size_gb   = 300
  ...
  depends_on = [render_registry_credential.ghcr, render_project.rapidly]
}
...
module "production" {
  ...
  render_environment_id  = render_project.rapidly.environments["Production"].id
  registry_credential_id = render_registry_credential.ghcr.id

  api_service_config = {
    allowed_hosts   = "[\"rapidly.tech\", \"backoffice.rapidly.tech\"]"
    cors_origins    = "[\"https://rapidly.tech\", \"https://github.com\", \"https://docs.rapidly.tech\"]"
    custom_domains  = [{ name = "api.rapidly.tech" }, { name = "api-alt.rapidly.tech" }, { name = "buy.rapidly.tech" }, { name = "backoffice.rapidly.tech" }]
  ...
  workers = {
    "scheduler" = {
      start_command      = "uv run python -m rapidly.worker.scheduler"
      plan               = "starter"
    ...
    "worker" = {
      start_command      = "uv run dramatiq rapidly.worker.run -p 2 -t 4 --queues low_priority"
      ...
      custom_domains     = [{ name = "worker.rapidly.tech" }]
      dramatiq_prom_port = "10000"
    }
    "worker-medium-priority" = {
      start_command      = "uv run dramatiq rapidly.worker.run -p 2 -t 4 --queues medium_priority"
    ...
    "worker-high-priority" = {
      start_command      = "uv run dramatiq rapidly.worker.run -p 2 -t 4 --queues high_priority"
  ...
  aws_s3_config = {
    ...
    files_public_bucket_name = "rapidly-public-files"
  }
  ...
  depends_on = [render_registry_credential.ghcr, render_project.rapidly, render_postgres.db, render_redis.redis]
}
